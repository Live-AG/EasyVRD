...
Функция СформироватьДанныеXML(ДанныеЗаполнения) Экспорт
	...
	ОбъектXDTO = СериализаторXDTO.ЗаписатьXDTO(ДанныеЗаполнения);
...
Процедура ЗаполнитьЭлемент_point(PointXDTO, ДанныеЗаполнения)
	...
	Если Лев(СтрокаСоединения, 1) = """" И Прав(СтрокаСоединения, 1) = """" Тогда
		ДлинаСтрокиПодключения = СтрДлина(СтрокаСоединения);
		СтрокаСоединения = Сред(СтрокаСоединения, 2, ДлинаСтрокиПодключения - 2);
	КонецЕсли;
	
	PointXDTO.base = "/" + ДанныеЗаполнения.ИмяБазы;
	PointXDTO.ib = """" + НормализованнаяСтрокаXML(СтрокаСоединения) + """";
...
Процедура ЗаполнитьЭлемент_openID(PointXDTO, ДанныеЗаполнения)
	...
	ИначеЕсли ДанныеЗаполнения.ИспользоватьКакOpenIDПровайдера = Истина Тогда
		...
		OpenIDlifetimeXDTO = ПолучитьОписаниеТипаXDTO("OpenIDConnectRP");
		...
		Для Каждого ЭлементПереадресации Из ДанныеЗаполнения.РазрешенныеАдресаПереадресации Цикл
			ProviderXDTO.returnto.Добавить(XMLСтрока(ЭлементПереадресации.Значение));
		КонецЦикла;
...
Функция ПолучитьAccessTokenAuthentication(ДанныеЗаполнения)
	...
	//TODO: Проверка Версии платформы
	Issuers	= ПолучитьОписаниеТипаXDTO("AccessTokenIssuers");
	Issuer	= ПолучитьОписаниеТипаXDTO("AccessTokenIssuer");
